...
static double c1_s_stab = 0.1; // mesh-only shear regularization; not used in omega*
static double kappa_vol = 100.0; // independent volumetric penalty; not tied to C1_S_STAB
...
static RegularizationMode regularization_mode = RegularizationMode::RAW_NH;
static int         s_reg_projection_bins = 0; // <=0 uses reference_profile_bins
...
// C1_S_STAB is mesh-only and never participates in this mapping.
...
// Full-section FE data for section-projected regularization.
static bool        s_reg_section_data_built = false;
static std::vector<double> s_reg_eta_bar;        // full-section centroid in eta
static std::vector<double> s_reg_I2_c;           // full-section second moment per unit s
static std::vector<double> s_reg_area_per_length;
static std::vector<double> s_reg_proj_dev_A;     // subtract A + B*eta_c from dev axial stress
static std::vector<double> s_reg_proj_dev_B;
static std::vector<double> s_reg_proj_dil_A;     // subtract A + B*eta_c from dil axial stress
...
inline double get_c1_s_stab_local_from_reference_point(const libMesh::Point&)
{
    return c1_s_stab;
}
...
inline RegularizationMode parse_regularization_mode(const std::string& mode_raw)
{
    const std::string mode = normalize_mode_string(mode_raw);
    if (mode == "RAW-NH" || mode == "RAW" || mode == "NEO-HOOKEAN")
        return RegularizationMode::RAW_NH;
    if (mode == "PROJECTED-NH" || mode == "PROJECTED" ||
        mode == "SECTION-PROJECTED" || mode == "SECTION-PROJECTED-NH")
        return RegularizationMode::PROJECTED_NH;

    TBOX_ERROR("Unknown REGULARIZATION_MODE = \"" << mode_raw
               << "\". Expected \"RAW_NH\" or \"PROJECTED_NH\".\n");
    return RegularizationMode::RAW_NH;
}

inline const char* regularization_mode_name()
{
    switch (regularization_mode)
    {
    case RegularizationMode::RAW_NH:
        return "RAW_NH";
    case RegularizationMode::PROJECTED_NH:
        return "PROJECTED_NH";
    }
    return "RAW_NH";
}

inline bool use_section_projected_regularization()
{
    return regularization_mode == RegularizationMode::PROJECTED_NH;
}
...
inline double reg_eta_bar_from_s_norm(const double s_norm)
{
    if (!s_reg_section_data_built || s_reg_eta_bar.empty()) return 0.0;
    const int nb = static_cast<int>(s_reg_eta_bar.size());
    const double t = clamp01(s_norm) * (nb - 1);
    const int i0 = std::min(static_cast<int>(t), nb - 2);
    const double alpha = t - static_cast<double>(i0);
    return (1.0 - alpha) * s_reg_eta_bar[i0] + alpha * s_reg_eta_bar[i0 + 1];
}

inline int reg_projection_bin_from_s_norm(const double s_norm)
{
    const int nb = std::max(3, s_reg_projection_bins > 0 ?
                            s_reg_projection_bins : reference_profile_bins);
    return std::min(nb - 1,
                    std::max(0, static_cast<int>(
                        clamp01(s_norm) * static_cast<double>(nb - 1) + 0.5)));
}
...
write_fe_active_section_diagnostic()
{
    ...
    out << "s_norm,eta_bar,I2_ideal,I2_c,I2_c_over_I2_ideal,eta_bar_over_h"
        << ",reg_eta_bar,reg_I2_c,reg_area_per_length,reg_eta_bar_over_h\n";
    out.setf(std::ios::scientific);
    out.precision(8);
    ...
    for (int i = 0; i < nb; ++i)
    {
        ...
        const double reg_ebar = (i < static_cast<int>(s_reg_eta_bar.size())) ?
            s_reg_eta_bar[i] : 0.0;
        const double reg_I2 = (i < static_cast<int>(s_reg_I2_c.size())) ?
            s_reg_I2_c[i] : 0.0;
        const double reg_area = (i < static_cast<int>(s_reg_area_per_length.size())) ?
            s_reg_area_per_length[i] : 0.0;
        const double reg_ebar_h = (h > 1.0e-12) ? reg_ebar / h : 0.0;
        out << s_norm << "," << s_fe_eta_bar[i] << "," << I2_ideal
            << "," << s_fe_I2_c[i] << "," << I2_ratio << "," << ebar_h
            << "," << reg_ebar << "," << reg_I2 << "," << reg_area
            << "," << reg_ebar_h << "\n";
    }
    out.flush();
...
build_fe_section_data(MeshBase& mesh)
{
    ...
    s_reg_eta_bar.assign(nb, 0.0);
    s_reg_I2_c.assign(nb, 0.0);
    s_reg_area_per_length.assign(nb, 0.0);
    std::vector<double> reg_sum_area(nb, 0.0);
    std::vector<double> reg_sum_eta(nb, 0.0);
    std::vector<double> reg_sum_eta2(nb, 0.0);
         ...
         el_it != mesh.active_local_elements_end(); ++el_it)
    {
        ...
        for (unsigned int qp = 0; qp < nq; ++qp)
        {
            ...
            reg_sum_area[bi] += jw;
            reg_sum_eta[bi]  += eta_qp * jw;
            reg_sum_eta2[bi] += eta_qp * eta_qp * jw;
    ...
    IBTK_MPI::sumReduction(reg_sum_area.data(), nb);
    IBTK_MPI::sumReduction(reg_sum_eta.data(),  nb);
    IBTK_MPI::sumReduction(reg_sum_eta2.data(), nb);
    ...
    for (int i = 0; i < nb; ++i)
    {
        ...
        const double ds_bin = std::max((s_norm_right - s_norm_left) *
                                       ...
                                       1.0e-12);
        if (reg_sum_area[i] > 1.0e-30)
        {
            s_reg_eta_bar[i] = reg_sum_eta[i] / reg_sum_area[i];
            const double I2_bin = std::max(0.0,
                reg_sum_eta2[i] - s_reg_eta_bar[i] * reg_sum_eta[i]);
            s_reg_I2_c[i] = I2_bin / ds_bin;
            s_reg_area_per_length[i] = reg_sum_area[i] / ds_bin;
        }
    ...
    int first_reg_valid = -1;
    for (int i = 0; i < nb; ++i)
    {
        if (s_reg_area_per_length[i] > 1.0e-30 && s_reg_I2_c[i] > 1.0e-30)
        {
            first_reg_valid = i;
            break;
        }
    }
    if (first_reg_valid >= 0)
    {
        for (int i = 0; i < first_reg_valid; ++i)
        {
            s_reg_eta_bar[i] = s_reg_eta_bar[first_reg_valid];
            s_reg_I2_c[i] = s_reg_I2_c[first_reg_valid];
            s_reg_area_per_length[i] = s_reg_area_per_length[first_reg_valid];
        }
        int last_valid = first_reg_valid;
        for (int i = first_reg_valid + 1; i < nb; ++i)
        {
            if (s_reg_area_per_length[i] > 1.0e-30 && s_reg_I2_c[i] > 1.0e-30)
            {
                const int next_valid = i;
                for (int j = last_valid + 1; j < next_valid; ++j)
                {
                    const double a = static_cast<double>(j - last_valid) /
                                     static_cast<double>(next_valid - last_valid);
                    s_reg_eta_bar[j] = (1.0 - a) * s_reg_eta_bar[last_valid] +
                                       a * s_reg_eta_bar[next_valid];
                    s_reg_I2_c[j] = (1.0 - a) * s_reg_I2_c[last_valid] +
                                    a * s_reg_I2_c[next_valid];
                    s_reg_area_per_length[j] =
                        (1.0 - a) * s_reg_area_per_length[last_valid] +
                        a * s_reg_area_per_length[next_valid];
                }
                last_valid = next_valid;
            }
        }
        for (int i = last_valid + 1; i < nb; ++i)
        {
            s_reg_eta_bar[i] = s_reg_eta_bar[last_valid];
            s_reg_I2_c[i] = s_reg_I2_c[last_valid];
            s_reg_area_per_length[i] = s_reg_area_per_length[last_valid];
        }
        s_reg_section_data_built = true;
    }
    else
    {
        s_reg_section_data_built = false;
    }
...
// =========================================================================
// Passive regularization PK1 stress.
//
// RAW_NH:
//   dev = 2*C1_S_STAB*(F - F^{-T})
//   dil = KAPPA_VOL*ln(J) F^{-T}
//
// C1_S_STAB is mesh-only and KAPPA_VOL is an independent near-incompressible
// penalty, so neither term changes the physical Euler-Bernoulli stiffness.
//
// PROJECTED_NH:
//   the raw stress is still used for local mesh stabilization, but the axial
//   fiber stress T_tt has its section resultant and bending moment removed:
//     T_clean = T_raw - (A(s) + B(s)*eta_c).
//   The correction is applied as a fiber PK1 stress
//     PP_corr = (A + B*eta_c) F (t0 tensor t0)
//   so the non-axial mesh-stabilizing parts of dev/dil remain intact.
// =========================================================================
static void
...
{
    const double c1_stab_local = get_c1_s_stab_local_from_reference_point(X_ref);
    PP = 2.0 * c1_stab_local * (FF - tensor_inverse_transpose(FF, NDIM));
}
                                   ...
                                   const libMesh::Point& X_ref)
{
    ...
    {
        TBOX_ERROR("Passive dilational regularization encountered non-positive "
                   "or non-finite det(F) = " << J
                   << " at X_ref=(" << X_ref(0) << ", " << X_ref(1)
                   << "). Section projection cannot recover an already "
                   "inverted element.\n");
    }
...
static void
apply_section_projected_regularization(TensorValue<double>& PP,
                                       const TensorValue<double>& FF,
                                       const ReferenceGeometrySample& ref_geom,
                                       const std::vector<double>& A_coeff,
                                       const std::vector<double>& B_coeff)
{
    if (!use_section_projected_regularization() || !s_reg_projection_ready)
        return;
    if (A_coeff.empty() || B_coeff.empty()) return;

    const double s_norm = clamp01(ref_geom.s / std::max(ref_arc_length, 1.0e-12));
    const int bi = std::min(static_cast<int>(A_coeff.size()) - 1,
                            std::max(0, reg_projection_bin_from_s_norm(s_norm)));
    if (bi >= static_cast<int>(B_coeff.size())) return;

    const double eta_c = ref_geom.eta - reg_eta_bar_from_s_norm(s_norm);
    const double T_corr = A_coeff[bi] + B_coeff[bi] * eta_c;

    TensorValue<double> t_t;
    outer_product(t_t, ref_geom.t_hat, ref_geom.t_hat);
    PP = PP - T_corr * FF * t_t;
}
                                    ...
                                    const ReferenceGeometrySample* ref_geom)
{
    ...
    if (ref_geom)
    {
        apply_section_projected_regularization(PP, FF, *ref_geom,
                                               s_reg_proj_dev_A,
                                               s_reg_proj_dev_B);
    }
}
...
void PK1_dev_stress_function(TensorValue<double>& PP,
                             ...
                             void*)
{
    if (use_section_projected_regularization())
    {
        const ReferenceGeometrySample ref_geom =
            reference_geometry_from_system_data(system_var_data);
        compute_passive_dev_PK1_stress_impl(PP, FF, X_ref, &ref_geom);
    }
    else
    {
        compute_passive_dev_PK1_stress_impl(PP, FF, X_ref, nullptr);
    }
}
                             ...
                             void*)
{
    if (use_section_projected_regularization())
    {
        const ReferenceGeometrySample ref_geom =
            reference_geometry_from_system_data(system_var_data);
        compute_passive_dil_PK1_stress_impl(PP, FF, X_ref, &ref_geom);
    }
    else
    {
        compute_passive_dil_PK1_stress_impl(PP, FF, X_ref, nullptr);
    }
}
...
struct RegularizationProjectionBin
{
    double S0 = 0.0;
    double S1 = 0.0;
    double S2 = 0.0;
...
static void
solve_regularization_projection_coefficients(
    const RegularizationProjectionBin& bin,
    const double N,
    const double M,
    double& A,
    double& B)
{
    A = 0.0;
    B = 0.0;
    const double det = bin.S0 * bin.S2 - bin.S1 * bin.S1;
    if (bin.S0 <= 1.0e-30 || det <= 1.0e-30) return;
    A = (N * bin.S2 - M * bin.S1) / det;
    B = (M * bin.S0 - N * bin.S1) / det;
}

static void
update_section_projected_regularization(
    Pointer<IBFEMethod>  ib_method_ops,
    MeshBase&            mesh,
    EquationSystems*     equation_systems)
{
    if (!use_section_projected_regularization())
    {
        s_reg_projection_ready = false;
        return;
    }
    if (!equation_systems) return;
    if (!s_reg_section_data_built)
    {
        TBOX_ERROR("PROJECTED_NH regularization requires full-section FE data. "
                   "Check the Laplace reference parameterization build.\n");
    }

    const unsigned int dim = mesh.mesh_dimension();
    const int nb = std::max(3, s_reg_projection_bins > 0 ?
                            s_reg_projection_bins : reference_profile_bins);
    std::vector<RegularizationProjectionBin> bins(static_cast<std::size_t>(nb));
    ...
    const libMesh::Order quad_order =
        Utility::string_to_enum<libMesh::Order>(s_reg_projection_quad_order);
    std::unique_ptr<FEBase> fe(FEBase::build(dim, fe_type));
         ...
         el_it != mesh.active_local_elements_end(); ++el_it)
    {
        ...
        for (unsigned int qp = 0; qp < qrule->n_points(); ++qp)
        {
            ...
            // Skip inverted/degenerate elements – they contribute no valid
            // data to the section projection and the dil stress would abort.
            const double J_qp = FF.det();
            if (!(J_qp > 1.0e-14) || !std::isfinite(J_qp)) continue;

            TensorValue<double> PP_dev(0.0), PP_dil(0.0);
            compute_raw_passive_dev_PK1_stress(PP_dev, FF, X_ref_qp);
            compute_raw_passive_dil_PK1_stress(PP_dil, FF, X_ref_qp);
            ...
            RegularizationProjectionBin& bin = bins[static_cast<std::size_t>(bi)];
            const double eta_c = ref_geom.eta - reg_eta_bar_from_s_norm(s_norm);
            const double jw = JxW[qp];
            const double T_dev =
                axial_fiber_stress_from_PK1(PP_dev, FF, ref_geom.t_hat);
            const double T_dil =
                axial_fiber_stress_from_PK1(PP_dil, FF, ref_geom.t_hat);

            bin.S0 += jw;
            bin.S1 += eta_c * jw;
            bin.S2 += eta_c * eta_c * jw;
            bin.N_dev += T_dev * jw;
            bin.M_dev += T_dev * eta_c * jw;
            bin.N_dil += T_dil * jw;
            bin.M_dil += T_dil * eta_c * jw;
        }
    }

    const int n_values_per_bin = 7;
    std::vector<double> reduced(static_cast<std::size_t>(nb * n_values_per_bin), 0.0);
    std::size_t r = 0;
    for (int i = 0; i < nb; ++i)
    {
        const RegularizationProjectionBin& bin = bins[static_cast<std::size_t>(i)];
        reduced[r++] = bin.S0;
        reduced[r++] = bin.S1;
        reduced[r++] = bin.S2;
        reduced[r++] = bin.N_dev;
    ...
    for (int i = 0; i < nb; ++i)
    {
        RegularizationProjectionBin& bin = bins[static_cast<std::size_t>(i)];
        bin.S0 = reduced[r++];
        bin.S1 = reduced[r++];
        bin.S2 = reduced[r++];
        bin.N_dev = reduced[r++];
    ...
    s_reg_proj_dev_A.assign(nb, 0.0);
    s_reg_proj_dev_B.assign(nb, 0.0);
    s_reg_proj_dil_A.assign(nb, 0.0);
    s_reg_proj_dil_B.assign(nb, 0.0);

    double max_raw_N = 0.0;
    double max_raw_M = 0.0;
    double max_post_N = 0.0;
    double max_post_M = 0.0;
    int n_valid = 0;
    for (int i = 0; i < nb; ++i)
    {
        const RegularizationProjectionBin& bin = bins[static_cast<std::size_t>(i)];
        double A_dev = 0.0, B_dev = 0.0, A_dil = 0.0, B_dil = 0.0;
        solve_regularization_projection_coefficients(bin, bin.N_dev, bin.M_dev,
                                                     A_dev, B_dev);
        solve_regularization_projection_coefficients(bin, bin.N_dil, bin.M_dil,
                                                     A_dil, B_dil);
        s_reg_proj_dev_A[i] = A_dev;
        s_reg_proj_dev_B[i] = B_dev;
        s_reg_proj_dil_A[i] = A_dil;
        s_reg_proj_dil_B[i] = B_dil;

        if (bin.S0 <= 1.0e-30) continue;
        ++n_valid;
        max_raw_N = std::max(max_raw_N,
            std::max(std::abs(bin.N_dev), std::abs(bin.N_dil)));
        max_raw_M = std::max(max_raw_M,
            std::max(std::abs(bin.M_dev), std::abs(bin.M_dil)));
        const double post_N_dev = bin.N_dev - A_dev * bin.S0 - B_dev * bin.S1;
        const double post_M_dev = bin.M_dev - A_dev * bin.S1 - B_dev * bin.S2;
        const double post_N_dil = bin.N_dil - A_dil * bin.S0 - B_dil * bin.S1;
        const double post_M_dil = bin.M_dil - A_dil * bin.S1 - B_dil * bin.S2;
        max_post_N = std::max(max_post_N,
            std::max(std::abs(post_N_dev), std::abs(post_N_dil)));
        max_post_M = std::max(max_post_M,
            std::max(std::abs(post_M_dev), std::abs(post_M_dil)));
    }

    if (n_valid <= 0)
    {
        TBOX_ERROR("PROJECTED_NH regularization failed: no valid section bins.\n");
    }
    s_reg_projection_ready = true;

    static bool printed_once = false;
    if (!printed_once && IBTK_MPI::getRank() == 0)
    {
        pout << "  PROJECTED_NH regularization initialized: bins = " << nb
             << ", quad_order = " << s_reg_projection_quad_order
             << ", first-pass max raw |N|/|M| = "
             << max_raw_N << " / " << max_raw_M
             << ", projected residual max |N|/|M| = "
             << max_post_N << " / " << max_post_M << "\n";
        printed_once = true;
    }
}
...
int main(int argc, char* argv[])
{
...

    {
        ...
        c1_s_caudal_transition_w =
            input_db->getDoubleWithDefault(
                ...
                input_db->getDoubleWithDefault("C1_S_CAUDAL_TRANSITION_W",
                                               c1_s_caudal_transition_w));
        c1_s_stab =
            input_db->getDoubleWithDefault("C1_S_STAB", c1_s_stab);
        kappa_vol =
        ...
        regularization_mode =
            parse_regularization_mode(input_db->getStringWithDefault(
                "REGULARIZATION_MODE", regularization_mode_name()));
        s_reg_projection_bins =
            input_db->getIntegerWithDefault("REG_PROJECTION_BINS",
                                            s_reg_projection_bins);
        s_reg_projection_quad_order =
            input_db->getStringWithDefault("REG_PROJECTION_QUAD_ORDER",
                                           s_reg_projection_quad_order);
        ...
        c1_s_stab = std::max(0.0, c1_s_stab);
        ...
        s_reg_projection_bins = std::max(0, s_reg_projection_bins);
        ...
        if (use_laplace_reference_parameterization &&
            (use_fe_active_section_data || use_section_projected_regularization()))
        {
            pout << "  Building FE-consistent section data "
                 << "(active eta_bar/I2_c and full-section regularization data)...\n";
            build_fe_section_data(mesh);
        }
        if (use_section_projected_regularization() && !s_reg_section_data_built)
        {
            TBOX_ERROR("REGULARIZATION_MODE=PROJECTED_NH requires full-section "
                       "regularization section data.\n");
        }
        ...
        pout << "  mesh regularization C1_S_STAB = " << c1_s_stab
             << " (C1_S_STAB / (4*C1_S_PHYSICS) = "
             << c1_s_stab / std::max(4.0 * c1_s_physics, 1.0e-30)
             << ")\n";
        pout << "  volumetric penalty KAPPA_VOL = " << kappa_vol << "\n";
        ...
        pout << "  passive regularization mode = "
             << regularization_mode_name()
             << ", projection bins = "
             << (s_reg_projection_bins > 0 ? s_reg_projection_bins :
                 reference_profile_bins)
             << ", projection quad_order = "
             << s_reg_projection_quad_order << "\n";
        ...
        update_section_projected_regularization(ib_method_ops, mesh,
                                                equation_systems);
               ...
               && time_integrator->stepsRemaining())
        {
            ...
            // NOTE: Do NOT call update_section_projected_regularization here
            // with X_new. The projection for the *next* step is recomputed
            // from X_current (= X_new) at the top of the next iteration anyway.
            // Calling it post-step reads potentially-inverted tail cap elements.
