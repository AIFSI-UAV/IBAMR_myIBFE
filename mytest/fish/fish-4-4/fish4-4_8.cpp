...
//
// Physics basis:
//   Xu & Yu (2023) Phys. Fluids 35, 071905
//   "Effects of body stiffness on propulsion during fish self-propelled swimming"
//   Thunniform prescribed-kinematics reference:
//     y/L = (0.016 - 0.066 ξ + 0.15 ξ²) sin(2π(ξ/1.25 - f t))
//     with ξ = (x_ref - x_head)/L.
//
...
//   Mm(s,t) = R(t) · env_s(s) · β_act · S_h(h)
//             · G_caudal(xi) · K_shape(xi) · cos(k_act*xi - omega*t + phi)
//
//   where xi is the active-body coordinate from ACTIVE_S_START to
//   ACTIVE_S_END on the current reference backbone. K_shape is selectable
//   through K_SHAPE_MODE.
//
...
//   where I₂_eff = ∫w(η)η²dη. With only a 2D fish outline available,
//   S_h(h) = h_ref^2*(h/h_ref)^p, where p is ACTIVE_MOMENT_H_POWER.
...
//
// Calibration guide (Section III of Xu & Yu):
//   β_act is adjusted until tail amplitude ≈ A_target = 0.10L.
//   Start with β_act = 2.5–10 and bisect to match A/L at cycle 4–6.
//   Optimal stiffness regime: ω/ω₀ ≈ 4 (medium stiffness, mode II).
// ---------------------------------------------------------------------
...
static double fluid_viscosity = 0.00960;
static double c1_s = 0.2; // fallback value for the spatial profile
static double c1_s_anterior = 0.2;
static double c1_s_peduncle = 0.2;
static double c1_s_caudal = 0.2;
...
static double structural_kv_loss_factor = 0.05; // eta_s*omega/(2*C1); 0 disables intrinsic damping
static double structural_kv_stress_cap_over_c1 = 50.0;

// --- Geometry / actuation frequency ---
static double fish_length = 1.10;
static double x_leading   = -0.10;
static double wave_frequency = 1.60;
static double wave_ramp_time = 2.0;
static double wave_omega     = 2.0 * M_PI * 1.60;
...
// -----------------------------------------------------------------------
// Phase-2: Active bending moment parameters  (Xu & Yu 2023)
// -----------------------------------------------------------------------
//
//   Active drive (MUSCLE_MOMENT, default; Xu & Yu 2023 Eq. 11-12):
//     Mm(s,t) = R(t) · env_s(s) · beta_act
//               · h_ref^2 · (h/h_ref)^p · K_shape(xi) · G_caudal(xi)
//               · cos(2*pi*xi/lambda_act - omega*t + phi0)
//     K_shape(xi) = 0.5 · (1 - cos(pi*xi))     for K_SHAPE_MODE = HALF-BELL
//                or 0.5 · (1 - cos(2*pi*xi))   for K_SHAPE_MODE = BELL.
//     xi = 0 at ACTIVE_S_START and xi = 1 at ACTIVE_S_END, both normalized
//     on the current reference backbone.
//     The baseline uses p = 3, i.e. h_scale(s)=h_ref^2*(h/h_ref)^p.
//
// β_act  : strength factor; bisect to match A/L ≈ 0.10.
//          Larger c1_s (stiffer body) requires larger β_act.

static double beta_act  = 30.0;    // strength factor β  [calibrate to target tail amplitude]
static double active_wavelength_over_L = 1.00; // muscle activation wavelength / active-body length
static double active_phase0 = 0.0; // φ_M; use pi to reverse curvature-drive sign
static double active_moment_to_stress_sign = 1.0; // test switch for Mm -> T_act sign chain
enum class ActiveKShapeMode { HALF_BELL, BELL };
static ActiveKShapeMode active_k_shape_mode = ActiveKShapeMode::HALF_BELL;
...
static double active_s_smooth = 0.08;  // smooth ramp length in s/L
static double active_band_fraction = 0.45; // lateral cosine band: outer fraction of half-thickness that is active
...
static double active_moment_h_power = 3.0; // h_scale(s)=h_ref^2*(h/h_ref)^power
...
static double laplace_tail_bc_width_over_L = 0.03;
...
inline double muscle_moment_shape_from_xi(const double xi_in)
{
    ...
    case ActiveKShapeMode::HALF_BELL:
        ...
        return 0.5 * (1.0 - std::cos(M_PI * xi));
    case ActiveKShapeMode::BELL:
        // Xu & Yu bell-shaped active moment: 0 at both ends, peak at xi=0.5.
...
// =========================================================================
// Active section consistency diagnostic (Xu & Yu 2023)
// =========================================================================
...
// =========================================================================
// Active bending moment PK1 stress   (Xu & Yu 2023; Xu, Zhou & Yu 2024)
//
                               ...
                               const double time)
{
    ...
    const double Mm_clamped = std::max(-Mm_max, std::min(Mm_max, Mm));
    const double T_act =
...
int main(int argc, char* argv[])
{
...

    {
        ...
        pout << "  active drive model = MUSCLE_MOMENT\n";
        ...
        pout << "  active moment-to-stress mapping = "
             ...
             << "(fallback uses eta/I2_eff); curvature-conjugate moment = "
        ...
        //   (4) Active bending       — Xu & Yu 2023 active muscle moment
        //
