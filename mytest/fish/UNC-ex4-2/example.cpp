                        ...
                        void* /*ctx*/)
{
    ...
    const double x_min = -0.50;   // set according to your mesh
    const double phase = 2.0 * M_PI * ((X(0) - x_min) / body_length + time);

    //const double xi = (X(0) - x_min) / body_length;
    //const double phi = 2.0 * M_PI * (xi + time);

    double T_bottom = 0.5 * T0 * (1.0 + std::sin(phase));
    double T_top    = 0.5 * T0 * (1.0 + std::sin(phase + M_PI));

    double T = (X(1) < 0.0) ? T_bottom : T_top;
