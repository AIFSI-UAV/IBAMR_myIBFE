...
// =========================================================================
// Soft passive shape-maintenance PK1 stress:
//
//   Pe = eta_tot * (F - F^{-T})
//
// This is intentionally small and is not the passive bending model. Bending is
// supplied by the Euler-Bernoulli section-resultant callback below.
// =========================================================================
static void
compute_passive_PK1_stress_impl(TensorValue<double>& PP,
                                const TensorValue<double>& FF)
{
    PP = eta_tot * (FF - tensor_inverse_transpose(FF, NDIM));
}

void PK1_passive_stress_function(
    ...
    void*)
{
    compute_passive_PK1_stress_impl(PP, FF);
}
...
force_decomp_component_name(const int c)
{
    switch (c)
    {
    case FORCE_PASSIVE:    return "passive";
    ...
    EquationSystems*     equation_systems)
{
         ...
         el_it != mesh.active_local_elements_end(); ++el_it)
    {
        ...
        for (unsigned int qp = 0; qp < qrule->n_points(); ++qp)
        {
            ...
            for (int c = FORCE_PASSIVE; c <= FORCE_SUM; ++c)
            {
                const double T_tt =
    ...
    for (int i = 0; i < nb; ++i)
    {
        ...
        const double M_passive = M_sec[FORCE_PASSIVE] +
                                      ...
                                      const double         vcm_y)
{
         ...
         el_it != mesh.active_local_elements_end(); ++el_it)
    {
        ...
        for (unsigned int qp = 0; qp < qrule->n_points(); ++qp)
        {
            ...
            TensorValue<double> PP_passive(0.0), PP_damping(0.0), PP_active(0.0);
            TensorValue<double> PP_eb_passive(0.0);
            compute_passive_PK1_stress_impl(PP_passive, FF);
            ...
            accumulate_qp(FORCE_PASSIVE, PP_passive, F_dot, JxW[qp], dphi_qp, elem_loads);
    ...
    if (std::isfinite(dt_eff) && dt_eff > 1.0e-12)
    {
        s_force_decomp_work_passive += acc[FORCE_PASSIVE].P * dt_eff;
        ...
        s_force_decomp_work_weak_passive += acc[FORCE_PASSIVE].P_weak * dt_eff;
    ...
    int dominant_L1 = FORCE_PASSIVE;
    int dominant_Pabs = FORCE_PASSIVE;
    for (int c = FORCE_EB_PASSIVE; c <= FORCE_ACTIVE; ++c)
    {
...
int main(int argc, char* argv[])
{
...

    {
        ...
        //   (1) Passive shape maintenance — eta_tot*(F - F^{-T})
        //   (2) EB passive bending        — Xu/Zhou/Yu section-resultant analogue
        //   (3) Structural damping        — legacy fiber Kelvin-Voigt damping
        ...
        IBFEMethod::PK1StressFcnData PK1_passive_data(
            PK1_passive_stress_function, no_sys_data);
        ...
        PK1_passive_data.quad_order =
            Utility::string_to_enum<libMesh::Order>(
                input_db->getStringWithDefault("PK1_PASSIVE_QUAD_ORDER", "FIFTH"));
        ...
        ib_method_ops->registerPK1StressFunction(PK1_passive_data);
        ...
        IBFEMethod::PK1StressFcnData pk1_passive_post_data(
            PK1_passive_stress_function, no_sys_data);
        ib_post_processor->registerTensorVariable(
            "sigma_passive_shape", MONOMIAL, CONSTANT,
            IBFEPostProcessor::cauchy_stress_from_PK1_stress_fcn,
            no_sys_data, &pk1_passive_post_data);
