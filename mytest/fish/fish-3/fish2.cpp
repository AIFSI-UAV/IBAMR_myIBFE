...
// target-only, fixed y, free global x translation and yaw, no
// self-propulsion, no active stress. The target still prescribes shape,
// while the global x/theta frame is refreshed from the current coupled
...
static double theta_fixed = 0.0;    // 参考构型姿态角；target 的 theta 由当前构型决定。
                              ...
                              double& ytar)
{
    ...
    // 5) map back to lab frame with fixed y and released x/theta
...
// target prescribes shape and fixed y, but not global x translation/yaw.
// -------------------------------
void
