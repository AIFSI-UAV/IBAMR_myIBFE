...
// ---------------------------------------------------------------------
// Fourth-stage kinematic audit:
// target-only, free global x/y translation and yaw, no
// self-propulsion, no active stress. The target still prescribes shape,
// while the global x/y/theta frame is refreshed from the current coupled
...
static double ycom_target_current = ycom_fixed;
static double theta_fixed = 0.0;    // 参考构型姿态角；target 的 x/y/theta 由当前构型决定。
static double theta_target_current = theta_fixed;
...
// -------------------------------
// Target generator with fixed reference pose and supplied target pose
                              ...
                              double& ytar)
{
    ...
    // 5) map back to lab frame with released x/y/theta
...
// target prescribes shape, but not global x/y translation/yaw.
// -------------------------------
void
...
double compute_current_xcom(EquationSystems* equation_systems,
                            ...
                            MeshBase& mesh);
double compute_current_ycom(EquationSystems* equation_systems,
                            const std::string& coords_system_name,
                            MeshBase& mesh);
double compute_current_theta(EquationSystems* equation_systems,
                             const std::string& coords_system_name,
                             MeshBase& mesh,
                             double theta_guess);
void update_target_pose_cache(EquationSystems* equation_systems,
                              const std::string& coords_system_name,
                              MeshBase& mesh);
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        update_target_pose_cache(equation_systems, coords_system_name, mesh);
...
inline double
wrap_angle_near(double theta, double theta_ref)
{
    while (theta - theta_ref > M_PI) theta -= 2.0 * M_PI;
    while (theta - theta_ref < -M_PI) theta += 2.0 * M_PI;
    return theta;
}
...
double
compute_current_ycom(EquationSystems* equation_systems, const std::string& coords_system_name, MeshBase& mesh)
{
    const unsigned int dim = mesh.mesh_dimension();
    System& X_system = equation_systems->get_system<System>(coords_system_name);
    NumericVector<double>* X_vec = X_system.solution.get();
...
double
compute_current_theta(EquationSystems* equation_systems,
                      const std::string& coords_system_name,
                      MeshBase& mesh,
                      double theta_guess)
{
    ...
    const double area_tot = local_first[0] > 0.0 ? local_first[0] : 1.0;
    const double xcom_current = local_first[1] / area_tot;
    const double ycom_current = local_first[2] / area_tot;

    const double c_ref = std::cos(theta_fixed);
    const double s_ref = std::sin(theta_fixed);
    ...
    for (auto el_it = mesh.active_local_elements_begin(); el_it != mesh.active_local_elements_end(); ++el_it)
    {
        const Elem* elem = *el_it;
        fe->reinit(elem);
        const std::vector<libMesh::Point>& X_qp = fe->get_xyz();
        for (unsigned int d = 0; d < NDIM; ++d) dof_map.dof_indices(elem, dof_indices[d], d);
        get_values_for_interpolation(X_node, *X_ghost_vec, dof_indices);
        ...
        for (unsigned int qp = 0; qp < n_qp; ++qp)
        {
            ...
            const double dx_ref = X_qp[qp](0) - xcom_fixed;
            const double dy_ref = X_qp[qp](1) - ycom_fixed;
            const double x_body =  c_ref * dx_ref + s_ref * dy_ref;
            const double y_body = -s_ref * dx_ref + c_ref * dy_ref;

            const double dx_cur = x(0) - xcom_current;
            const double dy_cur = x(1) - ycom_current;

            a11 += JxW[qp] * (x_body * dx_cur + y_body * dy_cur);
            a21 += JxW[qp] * (x_body * dy_cur - y_body * dx_cur);
        }
    }

    double local_second[2] = { a11, a21 };
    IBTK_MPI::sumReduction(local_second, 2);

    const double theta_raw = std::atan2(local_second[1], local_second[0]);
    return wrap_angle_near(theta_raw, theta_guess);
}

void
update_target_pose_cache(EquationSystems* equation_systems, const std::string& coords_system_name, MeshBase& mesh)
{
    xcom_target_current = compute_current_xcom(equation_systems, coords_system_name, mesh);
    ycom_target_current = compute_current_ycom(equation_systems, coords_system_name, mesh);
    theta_target_current = compute_current_theta(equation_systems, coords_system_name, mesh, theta_target_current);
}
