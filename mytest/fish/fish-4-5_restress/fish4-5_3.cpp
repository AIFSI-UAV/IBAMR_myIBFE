// IBFE hyperelastic continuum fish: Direct 2D dorsal-ventral tension-only
// muscle active stress.  Activation wave → emergent bending moment and curvature.
// Passive stiffness: raw deviatoric/dilatational PK1 stress + optional damping.
...
// --- Direct 2D dorsal-ventral tension-only muscle active stress ---
// τ_a(s,η,t) = τ_max(s) · g_m(|η|/h) · a(s,η,t)   [Cauchy tension, ≥ 0]
// Dorsal (η≥0):  a = G_a(s) · max(0,  sin φ)
// Ventral (η<0): a = G_a(s) · max(0, -sin φ)
// φ(s,t) = 2π ξ/λ − ω t + φ₀  (ξ = normalised active-body arc coordinate)
// PK1: P_a = J τ_a / λ_f² · (F f₀) ⊗ f₀
// Section bending moment M_a(s,t) and curvature are emergent outputs.
// Absolute nondimensional Cauchy fiber tension (NOT multiples of c1).
static double active_tension_max_body      = 2.0;
static double active_tension_max_peduncle  = 2.0;
static double active_tension_max_caudal    = 1.0;
static double active_tension_body_transition_s   = 0.60;
static double active_tension_body_transition_w   = 0.30;
static double active_tension_caudal_transition_s = 0.85;
static double active_tension_caudal_transition_w = 0.10;
...
// Muscle activation pattern — controls what the dorsal/ventral sides do in time.
//   OFF              (D0) τ_a = 0 everywhere; verify no spurious motion.
//   BILATERAL_STATIC (D1) Both sides: activation=1*ramp(t); verify M_a≈0, possible axial shrink.
//   DORSAL_STATIC    (D2) Dorsal (η≥0): activation=1*ramp(t); ventral=0. Static bend test.
//   TRAVELING        (D3) Standard: dorsal max(0,sinφ), ventral max(0,-sinφ). Default.
enum class MuscleActivationMode
{
    OFF,
    BILATERAL_STATIC,
    DORSAL_STATIC,
...
static MuscleActivationMode muscle_activation_mode = MuscleActivationMode::TRAVELING;
...
static double active_t_act_max_over_c1 = 20.0;  // hard cap: τ_a ≤ cap·c1_local
...
static bool        s_active_tension_profile_debug_enable = false;
static std::string s_active_tension_profile_debug_filename =
    "active_tension_profile_debug.csv";
...
inline double active_tension_max_local(const double s_norm)
{
    const double xi = clamp01(s_norm);
    const double w_body =
        smoothstep_cosine(xi, active_tension_body_transition_s,
                          active_tension_body_transition_w);
    const double w_caudal =
        smoothstep_cosine(xi, active_tension_caudal_transition_s,
                          active_tension_caudal_transition_w);
    return active_tension_max_body
           + (active_tension_max_peduncle - active_tension_max_body) * w_body
           + (active_tension_max_caudal - active_tension_max_peduncle) * w_caudal;
}

// Bundles the requested, cap, and actually-applied Cauchy fiber tension so
// that all diagnostic paths report consistent values.
struct ActiveTensionState
{
    double requested              = 0.0;
    double cap                    = 0.0;
    double applied                = 0.0;
    double applied_over_requested = 1.0;
};

inline ActiveTensionState
get_active_tension_state(const double s_norm, const double c1_local)
{
    ActiveTensionState st;
    st.requested = active_tension_max_local(s_norm);
    st.cap       = active_t_act_max_over_c1 * c1_local;
    st.applied   = std::min(st.requested, st.cap);
    st.applied_over_requested =
        (st.requested > 1.0e-14) ? st.applied / st.requested : 1.0;
    return st;
}
                                                  ...
                                                  const char* caller_name)
{
    ...
    if (n > 2)
    {
        ...
        // mesh-derived upper envelope used by the active band and diagnostics.
...
parse_active_envelope_mode(const std::string& mode_raw)
{
    ...
    TBOX_ERROR("Unknown ACTIVE_ENVELOPE_MODE = \"" << mode_raw
               << "\". Expected \"K_SHAPE\" or \"POSTERIOR_RAMP\".\n");
    return ActiveEnvelopeMode::K_SHAPE;
}
...
inline MuscleActivationMode
parse_muscle_activation_mode(const std::string& mode_raw)
{
    const std::string mode = normalize_mode_string(mode_raw);
    if (mode == "OFF")               return MuscleActivationMode::OFF;
    if (mode == "BILATERAL-STATIC")  return MuscleActivationMode::BILATERAL_STATIC;
    if (mode == "DORSAL-STATIC")     return MuscleActivationMode::DORSAL_STATIC;
    if (mode == "TRAVELING")         return MuscleActivationMode::TRAVELING;
    TBOX_ERROR("Unknown MUSCLE_ACTIVATION_MODE = \"" << mode_raw
               << "\". Expected \"OFF\", \"BILATERAL_STATIC\","
                  " \"DORSAL_STATIC\", or \"TRAVELING\".\n");
    return MuscleActivationMode::TRAVELING;
}

inline const char* muscle_activation_mode_name()
{
    switch (muscle_activation_mode)
    {
    case MuscleActivationMode::OFF:              return "OFF";
    case MuscleActivationMode::BILATERAL_STATIC: return "BILATERAL_STATIC";
    case MuscleActivationMode::DORSAL_STATIC:    return "DORSAL_STATIC";
    case MuscleActivationMode::TRAVELING:        return "TRAVELING";
    }
    return "TRAVELING";
}
...
inline double activation_longitudinal_shape_from_xi(const double xi_in)
{
...
// G_a(s): longitudinal activation envelope — zero outside active band,
// rises to 1 in the body region. Used directly in τ_a = τ_max · g_m · G_a · activation.
inline double activation_envelope_from_s_norm(
    const double s_norm_in)
{
    ...
    {
        return active_on_taper_from_s_norm(s_norm) *
               posterior_ramp_shape_from_xi(xi) *
               active_tail_taper_from_s_norm(s_norm);
    }

    return longitudinal_active_envelope(s_norm) *
           activation_longitudinal_shape_from_xi(xi);
}

struct DorsalVentralActivation
{
    double dorsal = 0.0;
    double ventral = 0.0;
};

inline DorsalVentralActivation
dorsal_ventral_activation_factors(const double s, const double time)
{
    DorsalVentralActivation activation;
    switch (muscle_activation_mode)
    {
    case MuscleActivationMode::OFF:
        break;
    case MuscleActivationMode::BILATERAL_STATIC:
        activation.dorsal = 1.0;
        activation.ventral = 1.0;
        break;
    case MuscleActivationMode::DORSAL_STATIC:
        activation.dorsal = 1.0;
        break;
    case MuscleActivationMode::TRAVELING:
    {
        const double sin_phi = std::sin(active_phase_angle_from_s(s, time));
        activation.dorsal = std::max(0.0, sin_phi);
        activation.ventral = std::max(0.0, -sin_phi);
        break;
    }
    }
    return activation;
...
inline double
signed_bending_activation_drive(const double s,
                                const double s_norm,
                                const double time)
{
    const DorsalVentralActivation activation =
        dorsal_ventral_activation_factors(s, time);
    return activation_envelope_from_s_norm(s_norm) *
           (activation.dorsal - activation.ventral) *
           wave_ramp(time);
}
...
// Writes tau_max(s), G_a(s), and mode-aware activation profiles to CSV.
static void
write_active_tension_profile_debug()
{
    if (IBTK_MPI::getRank() != 0 || !s_active_tension_profile_debug_enable) return;

    std::ofstream out(s_active_tension_profile_debug_filename.c_str());
    if (!out.is_open())
    {
        TBOX_WARNING("write_active_tension_profile_debug(): cannot open "
                     << s_active_tension_profile_debug_filename << "\n");
        return;
    }

    // Use a unit ramp at t=0 so the file describes the selected activation
    // pattern instead of the zero-stress startup state.
    out << "activation_mode,s_norm,s,x_ref,xi,h"
        << ",tau_requested,tau_cap,tau_applied,tau_applied_over_requested"
        << ",G_a"
        << ",band_weight_upper,band_weight_lower"
        << ",act_dorsal_t0_unit_ramp,act_ventral_t0_unit_ramp"
        << ",tau_dorsal_t0_unit_ramp,tau_ventral_t0_unit_ramp\n";
    out.setf(std::ios::scientific);
    out.precision(10);
    ...
    for (int i = 0; i < ns; ++i)
    {
        ...
        const double xi = active_xi_from_s_norm(s_norm);
        const double G_a = activation_envelope_from_s_norm(s_norm);
        const double c1_loc = get_c1_s_passive_local(s_norm);
        const ActiveTensionState ts = get_active_tension_state(s_norm, c1_loc);
        const DorsalVentralActivation activation =
            dorsal_ventral_activation_factors(s, 0.0);
        const double act_dorsal = G_a * activation.dorsal;
        const double act_ventral = G_a * activation.ventral;
        const double bw_upper = active_band_weight(h, h);
        const double bw_lower = active_band_weight(-h, h);

        out << muscle_activation_mode_name()
            << "," << s_norm
            << "," << s
            << "," << X_mid(0)
            << "," << xi
            ...
            << "," << ts.cap
            << "," << ts.applied
            << "," << ts.applied_over_requested
            << "," << G_a
            << "," << bw_upper
            << "," << bw_lower
            << "," << act_dorsal
            << "," << act_ventral
            << "," << ts.applied * bw_upper * act_dorsal
            << "," << ts.applied * bw_lower * act_ventral
            << "\n";
    }
    out.flush();
    pout << "  " << s_active_tension_profile_debug_filename
         << " written (mode-aware unit-ramp tau/G_a/activation profiles at t=0;"
            " not actual startup stress since wave_ramp(0)=0)\n";
}
                             ...
                             void*)
{
    ...
    const double s_norm = clamp01(ref_geom.s / std::max(ref_arc_length, 1.0e-12));
    compute_passive_dev_PK1_stress_impl(
        PP, FF, get_c1_s_passive_local(s_norm));
}
    ...
    const TensorValue<double>& F_dot)
{
    ...
    const double s_norm = clamp01(ref_geom.s / std::max(ref_arc_length, 1.0e-12));
    const double c1_local = get_c1_s_passive_local(s_norm);
    ...
    void*)
{
    ...
    const ReferenceGeometrySample ref_geom =
        reference_geometry_from_system_data(system_var_data);
    compute_continuum_damping_PK1_stress_impl(PP, FF, ref_geom, F_dot);
}

// =========================================================================
// Direct 2D dorsal-ventral tension-only muscle active PK1 stress.
// τ_a = τ_max(s) · g_m(|η|/h) · G_a(s) · a(s,η,t)   (Cauchy fiber tension)
// P_a = J · τ_a / λ_f² · (F f₀) ⊗ f₀
// =========================================================================
static void
                               ...
                               const double time)
{
    ...
    if (muscle_activation_mode == MuscleActivationMode::OFF) return;

    const double G_a = activation_envelope_from_s_norm(s_norm);
    if (G_a <= 0.0) return;

    const double ramp = wave_ramp(time);
    const DorsalVentralActivation side_activation =
        dorsal_ventral_activation_factors(s, time);
    const double activation = (ref_geom.eta >= 0.0) ?
        side_activation.dorsal : side_activation.ventral;
    if (activation <= 0.0) return;

    const double band_w = active_band_weight(ref_geom.eta, h);
    if (band_w <= 0.0) return;

    const double c1_local = get_c1_s_passive_local(s_norm);
    const ActiveTensionState ts = get_active_tension_state(s_norm, c1_local);
    const double tau_a = ts.applied * band_w * G_a * activation * ramp;
    if (tau_a <= 0.0) return;

    const double J = FF.det();
    if (!(J > 1.0e-14) || !std::isfinite(J)) return;

    const VectorValue<double> fiber = FF * ref_geom.t_hat;
    const double lambda_f2 = std::max(fiber * fiber, 1.0e-24);
    TensorValue<double> fiber_f0;
    outer_product(fiber_f0, fiber, ref_geom.t_hat);
    PP = (J * tau_a / lambda_f2) * fiber_f0;
}
                                  ...
                                  const double         y_cm)
{
    ...
    const bool phase_analysis_enabled =
        muscle_activation_mode == MuscleActivationMode::TRAVELING;
    ...
    if (update_accum)
    {
        ...
        if (dt_eff > 1.0e-12)
        {
            ...
            for (std::size_t k = 0; k < samples.size(); ++k)
            {
                const double act_signal_signed =
                    signed_bending_activation_drive(
                        samples[k].s, samples[k].s_norm, loop_time);

                if (phase_analysis_enabled && std::isfinite(kappa_body[k]))
                {
                    s_curvature_cos_accum[k] += kappa_body[k] * c_t * dt_eff;
                    s_curvature_sin_accum[k] += kappa_body[k] * s_t * dt_eff;
                }
                if (phase_analysis_enabled &&
                    std::abs(act_signal_signed) > 1.0e-30)
                {
                    s_activation_cos_accum[k] += act_signal_signed * c_t * dt_eff;
                    s_activation_sin_accum[k] += act_signal_signed * s_t * dt_eff;
                }
                    ...
                    std::isfinite(s_prev_curvature_body[k]))
                {
                    ...
                    const double p_density = act_signal_signed * kappa_dot;
            ...
            if (phase_analysis_enabled) ++s_curvature_phase_samples;
        }
    ...
    const double curvature_phase_slope = phase_analysis_enabled ?
        phase_slope_over_s_norm(samples, curvature_phase_unwrapped) : nan;
    const double active_phase_slope_abs = phase_analysis_enabled ?
        active_phase_slope_abs_over_s_norm() : nan;
    const double active_phase_slope_expected = phase_analysis_enabled ?
        wave_time_sign * active_phase_slope_abs : nan;
    ...
    if (!out.is_open())
    {
        ...
        out << "step,time,cycle,phase,phase_samples,phase_start_time,phase_accum_time"
            ...
            << ",A_body_norm"
            << ",kappa_body,kappa_body_L"
            << ",act_signal_signed,act_envelope"
            << ",tau_requested,tau_applied,tau_applied_over_requested"
            ...
            << ",activation_kappadot_proxy,activation_kappadot_positive_proxy"
            << ",activation_kappadot_integral,activation_kappadot_positive_integral"
            << "\n";
        out.flush();
    }
    ...
    for (std::size_t k = 0; k < samples.size(); ++k)
    {
        const double G_a = activation_envelope_from_s_norm(samples[k].s_norm);
        const double act_signal_signed =
            signed_bending_activation_drive(
                samples[k].s, samples[k].s_norm, loop_time);
        const double c1_samp = get_c1_s_passive_local(samples[k].s_norm);
        const ActiveTensionState ts_samp =
            get_active_tension_state(samples[k].s_norm, c1_samp);
        out << iteration_num
            << "," << loop_time
            ...
            << "," << ts_samp.requested
            << "," << ts_samp.applied
            << "," << ts_samp.applied_over_requested
            << "," << curvature_phase[k]
            << "," << activation_phase[k]
            << "," << phase_lag[k]
...
// Returns â·(P f₀) = (a·P f₀)/λ_f, the nominal traction along the current
// fiber direction on the reference-normal face.  Integrating over reference
// area gives the correct section force/moment in mixed-basis form.
inline double
axial_nominal_traction_from_PK1(const TensorValue<double>& PP,
                                 const TensorValue<double>& FF,
                                 const VectorValue<double>& f0)
{
    const VectorValue<double> a = FF * f0;
    const double lambda_f = std::sqrt(std::max(a * a, 0.0));
    if (lambda_f <= 1.0e-12) return 0.0;
    return (a * (PP * f0)) / lambda_f;
}
    ...
    EquationSystems*     equation_systems)
{
         ...
         el_it != mesh.active_local_elements_end(); ++el_it)
    {
        ...
        for (unsigned int qp = 0; qp < qrule->n_points(); ++qp)
        {
            ...
            compute_continuum_damping_PK1_stress_impl(
                PP[FORCE_DAMPING], FF, ref_geom, F_dot);
            compute_active_PK1_stress_impl(
                PP[FORCE_ACTIVE], FF, ref_geom, loop_time);
            ...
            for (int c = FORCE_DEV; c <= FORCE_SUM; ++c)
            {
                const double T_nominal =
                    axial_nominal_traction_from_PK1(PP[c], FF, ref_geom.t_hat);
                bin.N[c] += T_nominal * jw;
                bin.M[c] += -T_nominal * ref_geom.eta * jw;
            }
        }
    }
    ...
    if (!out.is_open())
    {
        ...
        out << ",M_passive,M_resist,act_signal_signed"
            << ",tau_requested,tau_applied,tau_applied_over_requested"
            << ",R_active_resist,R_mag,C_oppose,R_internal_moment_residual\n";
        out.flush();
    }
    ...
    for (int i = 0; i < nb; ++i)
    {
        ...
        const double act_signal_signed =
            signed_bending_activation_drive(s_model, s_model_norm, loop_time);
        const double c1_sec = std::isfinite(c1_mean) ? c1_mean :
                              get_c1_s_passive_local(s_model_norm);
        const ActiveTensionState ts_sec = get_active_tension_state(s_model_norm, c1_sec);
                                      ...
                                      const double         vcm_y)
{
         ...
         el_it != mesh.active_local_elements_end(); ++el_it)
    {
        ...
        for (unsigned int qp = 0; qp < qrule->n_points(); ++qp)
        {
            ...
            const double s_norm = clamp01(ref_geom.s /
                                          std::max(ref_arc_length, 1.0e-12));
            compute_passive_dev_PK1_stress_impl(
                PP_dev, FF, get_c1_s_passive_local(s_norm));
            compute_passive_dil_PK1_stress_impl(PP_dil, FF, X_ref_qp);
            compute_continuum_damping_PK1_stress_impl(
                PP_damping, FF, ref_geom, F_dot);
            compute_active_PK1_stress_impl(
                PP_active, FF, ref_geom, loop_time);
    ...
    if (!out.is_open())
    {
        ...
        out << ",F_weak_sum_error_x,F_weak_sum_error_y"
            ...
            << ",P_active_internal,P_muscle_to_body"
            << ",W_active_internal,W_muscle_to_body"
...
int main(int argc, char* argv[])
{
...

    {
        ...
        // ── Direct 2D muscle τ_max profile ─────────────────────────────────
        active_tension_max_body =
            input_db->getDoubleWithDefault("ACTIVE_TENSION_MAX_BODY",
                                           active_tension_max_body);
        active_tension_max_peduncle =
            input_db->getDoubleWithDefault("ACTIVE_TENSION_MAX_PEDUNCLE",
                                           active_tension_max_peduncle);
        active_tension_max_caudal =
            input_db->getDoubleWithDefault("ACTIVE_TENSION_MAX_CAUDAL",
                                           active_tension_max_caudal);
        active_tension_body_transition_s =
            input_db->getDoubleWithDefault("ACTIVE_TENSION_BODY_TRANSITION_S",
                                           active_tension_body_transition_s);
        active_tension_body_transition_w =
            input_db->getDoubleWithDefault("ACTIVE_TENSION_BODY_TRANSITION_W",
                                           active_tension_body_transition_w);
        active_tension_caudal_transition_s =
            input_db->getDoubleWithDefault("ACTIVE_TENSION_CAUDAL_TRANSITION_S",
                                           active_tension_caudal_transition_s);
        active_tension_caudal_transition_w =
            input_db->getDoubleWithDefault("ACTIVE_TENSION_CAUDAL_TRANSITION_W",
                                           active_tension_caudal_transition_w);
        ...
        muscle_activation_mode =
            parse_muscle_activation_mode(input_db->getStringWithDefault(
                "MUSCLE_ACTIVATION_MODE", muscle_activation_mode_name()));
        posterior_g0 =
        ...
        laplace_tail_bc_width_over_L =
            input_db->getDoubleWithDefault("LAPLACE_TAIL_BC_WIDTH_OVER_L",
                                           laplace_tail_bc_width_over_L);
        if (active_tension_max_body < 0.0 ||
            active_tension_max_peduncle < 0.0 ||
            active_tension_max_caudal < 0.0)
        {
            TBOX_ERROR("ACTIVE_TENSION_MAX_BODY/PEDUNCLE/CAUDAL must be non-negative.\n");
        }
        ...
        active_band_fraction = clamp01(active_band_fraction);
        ...
        const bool legacy_active_profile_debug_enable =
            input_db->getBoolWithDefault(
                "ACTIVE_MOMENT_PROFILE_DEBUG_ENABLE",
                s_active_tension_profile_debug_enable);
        s_active_tension_profile_debug_enable =
            input_db->getBoolWithDefault(
                "ACTIVE_TENSION_PROFILE_DEBUG_ENABLE",
                legacy_active_profile_debug_enable);
        const std::string legacy_active_profile_debug_filename =
            input_db->getStringWithDefault(
                "ACTIVE_MOMENT_PROFILE_DEBUG_FILENAME",
                s_active_tension_profile_debug_filename);
        s_active_tension_profile_debug_filename =
            input_db->getStringWithDefault(
                "ACTIVE_TENSION_PROFILE_DEBUG_FILENAME",
                legacy_active_profile_debug_filename);
        ...
        pout << "\n=== IBFE continuum fish: direct 2D dorsal-ventral tension-only muscle ===\n";
        ...
        pout << "  passive C1 profile = "
             ...
             << c1_s_passive_peduncle << " / "
             << c1_s_passive_caudal << "\n";
        pout << "  passive C1 samples s/L = 0, 0.45, 0.65, 0.82, 0.86, 0.90, 1: "
        ...
        pout << "  continuum structural damping = "
             ...
             << " * C1_S_PASSIVE(s)\n";
        pout << "  active model = DORSAL_VENTRAL_TENSION_ONLY"
             << ", muscle_activation_mode = " << muscle_activation_mode_name()
             << ", initial bend amplitude = " << initial_bend_amplitude << "\n";
        pout << "  P_a = J*tau_a/lambda_f^2 * (F f0) x f0\n";
        pout << "  tau_a = tau_max(s)*g_m(eta/h)*G_a(s)*activation(eta,phi)*ramp(t)\n";
        pout << "  OFF: tau_a=0 | BILATERAL_STATIC: act=1 both sides"
                " | DORSAL_STATIC: act=1 eta>=0 only"
                " | TRAVELING: dorsal max(0,sinphi), ventral max(0,-sinphi)\n";
        pout << "  tau_max profile body/peduncle/caudal = "
             << active_tension_max_body << " / "
             << active_tension_max_peduncle << " / "
             << active_tension_max_caudal << "\n";
        pout << "  tau_max hard cap = " << active_t_act_max_over_c1 << " * C1_S_PASSIVE(s)\n";
        ...
        pout << "  bending moment M_a(s,t) and curvature kappa(s,t) are emergent outputs\n";
        ...
        pout << "  active tension profile debug = "
             << (s_active_tension_profile_debug_enable ? "on" : "off")
             << ", file = " << s_active_tension_profile_debug_filename << "\n";
