// IBFE hyperelastic continuum fish with internal active bending moment.
// Passive stiffness is supplied by raw deviatoric/dilatational PK1 stress,
// with optional continuum structural damping.
...
// --- Optional hybrid active model ---
// B(s) is an active moment scaling reference only. The raw dev/dil passive
// stiffness above is not calibrated from these values.
static bool   use_b_kappa_target_active_moment = false;
...
static bool   use_fe_normalized_stress = false;
...
static double active_kappa_amp = 0.5;
...
static double active_t_act_max_over_c1 = 100.0;
...
inline double get_c1_s_passive_local(const double x_norm)
{
...
{
    return get_c1_s_passive_local(reference_x_norm_from_point(X_ref));
}
...
inline double active_curvature_envelope_cap_from_s_norm(
    const double s_norm)
{
    if (!use_b_kappa_target_active_moment ||
                                                  ...
                                                  const double time)
{
    ...
    return ramp * beta_act * w_local * w_local;
}
                                                            ...
                                                            const double h)
{
    ...
    const double w_local = std::max(h, 0.0);
    return beta_act * w_local * w_local;
}

inline double active_traveling_amplitude()
{
    return use_b_kappa_target_active_moment ? active_kappa_amp : beta_act;
}
...
{
    return use_fe_normalized_stress ?
        fe_normalized_section_q_abs_max(s) :
        legacy_active_section_q_abs_max(s);
}
                                                ...
                                                const double s)
{
    return active_moment_cap_data(
        moment, s,
        get_c1_s_passive_local(reference_x_norm_from_s(s))).moment_applied;
}
...
write_active_moment_profile_debug()
{
    ...
    for (int i = 0; i < ns; ++i)
    {
        ...
        lower.s = s;
        lower.eta = -h;
        const double q_FE_upper =
            use_fe_normalized_stress ?
            fe_normalized_section_q(upper) :
            std::numeric_limits<double>::quiet_NaN();
        const double q_FE_lower =
            use_fe_normalized_stress ?
            fe_normalized_section_q(lower) :
            std::numeric_limits<double>::quiet_NaN();
        const double selected_q_upper =
            use_fe_normalized_stress ?
            q_FE_upper : legacy_active_section_q(upper);
        const double selected_q_lower =
            use_fe_normalized_stress ?
            q_FE_lower : legacy_active_section_q(lower);
        const ActiveMomentCapData cap = active_moment_cap_data(
            Mm0, s, get_c1_s_passive_local(reference_x_norm_from_s(s)));
        const double selected_sign =
            use_fe_normalized_stress ?
            -active_moment_to_stress_sign :
            active_moment_to_stress_sign;
        ...
        const double T_FE_upper =
            use_fe_normalized_stress ? T_upper :
            std::numeric_limits<double>::quiet_NaN();
        const double T_FE_lower =
            use_fe_normalized_stress ? T_lower :
            std::numeric_limits<double>::quiet_NaN();
                                ...
                                EquationSystems* equation_systems)
{
    ...
    if (!use_fe_normalized_stress) return;
    if (!equation_systems)
    {
                               ...
                               const double time)
{
    ...
    double q = 0.0;
    double moment_to_stress_sign = active_moment_to_stress_sign;
    if (use_fe_normalized_stress)
    {
        q = fe_normalized_section_q(ref_geom);
        // FE q satisfies -int(q*eta)dA=1.
        moment_to_stress_sign = -active_moment_to_stress_sign;
    }
    else
    {
        q = legacy_active_section_q(ref_geom);
    }
    if (std::abs(q) <= 1.0e-30) return;
    const double T_act =
        moment_to_stress_sign * cap.moment_applied * q;
...
int main(int argc, char* argv[])
{
...

    {
        ...
        use_b_kappa_target_active_moment =
        ...
        use_fe_active_section_data =
            input_db->getBoolWithDefault("USE_FE_ACTIVE_SECTION_DATA",
                                         use_fe_active_section_data);
        use_fe_normalized_stress =
            input_db->getBoolWithDefault("FE_NORMALIZED_STRESS",
                                         use_fe_normalized_stress);
        ...
        if (beta_act < 0.0)
        {
            TBOX_ERROR("BETA_ACT must be non-negative.\n");
        }
        ...
        if (s_beta_cal_enable && use_b_kappa_target_active_moment)
        {
            TBOX_ERROR("BETA_ACT_CALIBRATION cannot be used with "
                       "B_KAPPA_TARGET_ACTIVE_MOMENT=TRUE because BETA_ACT "
                       "does not control that branch.\n");
        }
        ...
        pout << "\n=== IBFE continuum fish: raw dev/dil + active bending ===\n";
        ...
        pout << "  passive C1 profile = "
             ...
             << c1_s_passive_peduncle << " / "
             << c1_s_passive_caudal << "\n";
        pout << "  passive C1 samples x/L = 0, 0.45, 0.65, 0.82, 0.86, 0.90, 1: "
        ...
        pout << "  B-kappa target active moment = "
             << (use_b_kappa_target_active_moment ? "ON" : "OFF")
        ...
        {
            pout << "  active moment = beta_act*w(s)^2*G_active(s)"
                    "*cos(active phase) [legacy]\n";
        }
        ...
        pout << "  FE-normalized stress = "
             << (use_fe_normalized_stress ? "ON" : "OFF");
        if (use_fe_normalized_stress)
        {
            pout << ", bins = " << fe_normalized_stress_bins
        ...
        pout << "  FE active section correction = "
             ...
             << (use_fe_normalized_stress ? " (bypassed by FE_NORMALIZED_STRESS)" : "")
             << "\n";
        if (!use_fe_normalized_stress && active_section_correction_enabled())
        {
            pout << "  FE section I2 floor ratio = "
                 << fe_section_i2_floor_ratio
                 << " (I2_use = max(I2_c_FE_scaled, ratio*I2_ideal))\n";
        }
        if (use_fe_normalized_stress)
        {
            pout << "  active stress = "
        ...
        else if (active_section_correction_enabled())
        {
