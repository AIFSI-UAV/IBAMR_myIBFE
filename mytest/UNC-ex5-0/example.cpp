...
{
    // --------------------------------------------------
    // 1. Reference point on the body (in reference coordinates)
    //    Choose this carefully: center, leading edge, hinge point, etc.
    // --------------------------------------------------
    ...
    // --------------------------------------------------
    // 2. Prescribed path of the reference point
    //    Example here: circular path
    //
    //    x_c(t) = Rc cos(omega t)
    //    y_c(t) = Rc sin(omega t)
    // --------------------------------------------------
    ...
    const double xc0 = ramp * U * time;
    const double xc1 = ramp * A * std::sin(omega * time);

    // --------------------------------------------------
    // 3. Tangent velocity of the path
    //    dx_c/dt, dy_c/dt
    // --------------------------------------------------
    // const double vx = -Rc * omega * std::sin(omega * time);
    // const double vy =  Rc * omega * std::cos(omega * time);
    const double vx = ramp * U;
    const double vy = A * omega * std::cos(omega * time);


    // --------------------------------------------------
    // 4. Heading angle follows tangent direction
    // --------------------------------------------------
    ...
    // --------------------------------------------------
    // 5. Relative position in reference configuration
    // --------------------------------------------------
    const double dX0 = X(0) - Xref0;
    const double dX1 = X(1) - Xref1;

    // --------------------------------------------------
    // 6. Rigid-body target position
    //    X_target = x_c(t) + R(theta) * (X - X_ref)
    // --------------------------------------------------
